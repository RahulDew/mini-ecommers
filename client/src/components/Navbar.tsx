      ...
      <div className="w-full h-[60px] flex justify-between items-center font-semibold text-base ">
        ...
        <div className="hidden md:flex justify-center items-center gap-2 lg:gap-5 bg-white shadow-2xl text-sm lg:text-base p-1.5 px-2 rounded-full">
          {NavbarData.navLinks.map((navLink, index) => {
            if (navLink.name === "Home" && user?.role !== "admin") {
              return null;
            }
            return (
              <NavLink
                ...
                className={({ isActive, isPending }) =>
                  isActive
                    ? "bg-indigo-600 text-white w-[80px] lg:w-[100px] text-center  p-1.5 px-2 rounded-full hover:text-primary duration-300"
                    : "bg-white text-black hover:bg-indigo-600 hover:text-white w-[80px] lg:w-[100px] text-center  p-1.5 px-2 rounded-full hover:text-primary duration-300"
                }
              >
                {navLink.name}
              </NavLink>
